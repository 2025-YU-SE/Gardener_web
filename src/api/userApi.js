...
export const attendance = () => {
  return api.post("user/attendance");
};
...
export const getUserRecentPosts = (userId) => {
  return api.get(`user/${userId}/posts/recent`);
};
...
export const getUserRecentFeedbacks = (userId) => {
  return api.get(`user/${userId}/feedbacks/recent`);
};

// 유저 피드백 목록 (Pageable)
export const getUserFeedbacks = (userId, page = 0, size = 10) => {
  return api.get(`user/${userId}/feedbacks`, {
    params: { page, size },
  });
};
...
export const getUserRecentScraps = (userId) => {
  return api.get(`user/${userId}/scraps/recent`);
};

// 유저 스크랩 목록 (Pageable)
export const getUserScraps = (userId, page = 0, size = 10) => {
  return api.get(`user/${userId}/scraps`, {
    params: { page, size },
  });
};
...
export const deleteUserAsAdmin = (userId) => {
  return api.delete(`user/${userId}/admin`);
};
