...
export default function Banner({
  ...
  return (
    <section className="w-full min-h-[280px] bg-[#E9FAEE] flex items-center justify-center py-6 sm:py-8">
      <div className="w-full max-w-[1000px] flex flex-col lg:flex-row items-center lg:items-center justify-between gap-6 lg:gap-8 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-center gap-4 sm:gap-6 lg:gap-10 w-full lg:w-auto">
          {/* 프로필 */}
          <img
            src={profileImg}
            alt="avatar"
            className="w-[100px] h-[100px] sm:w-[120px] sm:h-[120px] rounded-full object-cover ring-1 ring-[#D6EBD9] bg-[#E9FAEE] shrink-0"
            onError={(e) => {
          ...
          <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
            <div className="text-xl sm:text-2xl lg:text-[24px] font-bold">
              <span className="text-[#13B358]">{name}</span>님, 안녕하세요!
            </div>
            <div className="mt-1 text-sm sm:text-base lg:text-[15px] leading-6 text-[#2B2B2B]">
              코드와 피드백이 자라는 정원
              <br />
            ...
            <button
              ...
              className={`
                mt-4 inline-flex items-center justify-center px-4 sm:px-5 py-2 rounded-full
                text-white text-xs sm:text-sm lg:text-[14px] font-semibold
                transition
                ${
        ...
        <div className="flex flex-col gap-2 sm:gap-3 font-semibold w-full sm:w-auto">
          <MetricRow
            icon={<TbMessage2Check size={16} className="sm:w-[18px] sm:h-[18px] text-[#170000]" />}
            label="피드백 채택률"
            value={`${acceptRate}%`}
          />
          <MetricRow
            icon={<TbPencil size={16} className="sm:w-[18px] sm:h-[18px] text-[#170000]" />}
            label="등록한 게시물 수"
            value={postCount ?? 0}
          />
          <MetricRow
            icon={<VscFeedback size={16} className="sm:w-[18px] sm:h-[18px] text-[#170000]" />}
            label="등록한 피드백 수"
...
function MetricRow({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between gap-4 sm:gap-6 lg:gap-8 w-full sm:w-auto sm:min-w-[260px]">
      <div className="flex items-center gap-2 text-xs sm:text-sm lg:text-[14px] text-[#1E1E1E]">
        {icon}
        <span>{label}</span>
      </div>
      <div className="text-xs sm:text-sm lg:text-[14px] font-semibold text-[#1E1E1E] shrink-0">{value}</div>
    </div>
  );
}
